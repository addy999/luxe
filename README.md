...
![Luxe v0.1.3](docs/img/v0.1.3.png)
...
`extension/dt_link_flags.json` is generated per machine: it lists the include paths, link libraries, and preprocessor defines extracted from your own darktable build. `SConstruct` reads it, so regenerate it from your build tree whenever you rebuild darktable with different options. (The defines matter for correctness, not just compilation: darktable's headers put struct fields behind `#ifdef` guards, so a mismatched define set shifts struct offsets silently.)
