...
# Crop: render full frame, apply a centered 50% crop via set_crop(), and assert
# the cropped buffer is materially smaller AND its content matches the full
# frame at the crop-box origin (proves actual cropping, not just a dimension
# change).
# Run: Godot --headless --path godot-poc/project --script res://tests/crop_smoke_test.gd -- <input_raw> <out_dir>  (DT_BACKEND_DATADIR/DT_BACKEND_MODULEDIR must be exported)
...
func _initialize() -> void:
	...
	# Same edit scale (0.5) on both renders, so the ~half ratio must hold.
